...
validate_proxy() {
    ...
    local tmpresult=$(echo "$1" | grep -P '^(socks|socks4|socks5|http)://([^:]+:[^@]+@)?((([0-9]{1,3}\.){3}[0-9]{1,3})|(([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,})|(\[[0-9a-fA-F:]+\]|([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|((([0-9a-fA-F]{1,4}:){1,6})|::(([0-9a-fA-F]{1,4}:){1,6}))([0-9a-fA-F]{1,4}))):(0|[1-9][0-9]{0,4})$')
    if [ -z "$tmpresult" ]; then
...
get_ip_info() {
    ...
    # 从返回结果中提取国家代码，例如 "US"、"HK" 等
    LOCAL_COUNTRY=$(echo "$get_local_isp" | grep -woP '"country_code"\s*:\s*"\K[^"]+')
...
function showNetworkInfo() {
    ...
    get_ip_info
    if [ "$LANGUAGE" == 'en' ]; then
        echo -e " ${Font_SkyBlue}** Your Network Provider: ${LOCAL_ISP} (${LOCAL_IP_ASTERISK}) Country: ${LOCAL_COUNTRY}${Font_Suffix}"
        echo -e " ${Font_SkyBlue}** The script will automatically detect ${LOCL_COUNTRY} streaming media unlocking ${Font_Suffix}"
    else
        echo -e " ${Font_SkyBlue}** 您的网络为: ${LOCAL_ISP} (${LOCAL_IP_ASTERISK}) 国家地区为：${LOCAL_COUNTRY}${Font_Suffix} "
        echo -e " ${Font_SkyBlue}** 脚本将自动检测${LOCAL_COUNTRY}流媒体解锁${Font_Suffix}"
    fi
    echo ''
}
...
function runScript() {
    ...
    if [ "$USE_IPV4" -eq 1 ]; then
        Global_UnlockTest
        # 根据ip定位所在国家来判断脚本应该进行哪个国家的流媒体解锁
        case "$LOCAL_COUNTRY" in
            TW)
                TW_UnlockTest
                ;;
            HK)
            ...
            AR|BO|BR|CL|CO|EC|GY|PY|PE|SR|UY|VE)
                # 南美检测：阿根廷、玻利维亚、巴西、智利、哥伦比亚、厄瓜多尔、圭亚那、巴拉圭、秘鲁、苏里南、乌拉圭、委内瑞拉
                SA_UnlockTest
                ;;
            BN|KH|ID|LA|MY|MM|PH|SG|TH|TL|VN)
                # 东南亚检测：文莱、柬埔寨、印度尼西亚、老挝、马来西亚、缅甸、菲律宾、新加坡、泰国、东帝汶、越南
                SEA_UnlockTest
                ;;
            OA)
                OA_UnlockTest
            ...
            KR)
                KR_UnlockTest
                ;;
            AD|AL|AM|AT|AZ|BA|BE|BG|BY|CH|CY|CZ|DE|DK|EE|ES|FI|FO|FR|GB|GE|GI|GR|HR|HU|IE|IS|IT|LI|LT|LU|LV|MC|MD|ME|MK|MT|NL|NO|PL|PT|RO|RS|RU|SE|SI|SK|SM|TR|UA|VA)
                # 欧洲检测：完整的欧洲国家代码列表
                EU_UnlockTest
                ;;
            *)
                echo -e "${Font_Yellow}未匹配到特定国家，默认执行全球解锁检测${Font_Suffix}"
                ;;
        esac
